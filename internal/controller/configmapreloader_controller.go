...
import (
	...
	appsv1alpha1 "github.com/drumont/configmap-reloader/api/v1alpha1"
)
...
// +kubebuilder:rbac:groups=apps.drumont,resources=configmapreloaders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apps.drumont,resources=configmapreloaders/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=apps.drumont,resources=configmapreloaders/finalizers,verbs=update
...
func (r *ConfigMapReloaderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	configMapReloader := &appsv1alpha1.ConfigMapReloader{}
	if err := r.Get(ctx, req.NamespacedName, configMapReloader); err != nil {
		return ctrl.Result{}, client.IgnoreNotFound(err)
	}
	// Fetch the source ConfigMap
	sourceConfigMap := &corev1.ConfigMap{}
	sourceConfigMapName := types.NamespacedName{
		Namespace: configMapReloader.Spec.SourceNamespace,
		Name:      configMapReloader.Spec.ConfigMapName,
	}
	...
	destinationConfigMapName := types.NamespacedName{
		Namespace: configMapReloader.Spec.DestinationNamespace,
		Name:      configMapReloader.Spec.ConfigMapName,
	}
	if err := r.Get(ctx, destinationConfigMapName, destinationConfigMap); err != nil {
		if errors.IsNotFound(err) {
			log.Info("Creating ConfigMap in destination namespace", "Namespace", configMapReloader.Spec.DestinationNamespace)
			destinationConfigMap = &corev1.ConfigMap{
		...
		log.Info("Updating ConfigMap in destination namespace", "Namespace", configMapReloader.Spec.DestinationNamespace)
...
func (r *ConfigMapReloaderReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&appsv1alpha1.ConfigMapReloader{}).
